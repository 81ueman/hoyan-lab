...
checks that exercise deny, permit, multi-port permit, and live-probe coverage.

Examples:

```bash
go run ./cmd/hoyan model packet-classes --lab labs/acl-semantics --prefix 10.4.0.0/16
go run ./cmd/hoyan compare labs/acl-semantics/hoyan.clab.yml labs/acl-semantics/snapshots/actual.json --left-type model --right-type snapshot
```
