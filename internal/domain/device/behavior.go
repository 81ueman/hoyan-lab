...
type DeviceBehavior interface {
	...
	RouteValidForRIB(device model.Node, route domainroute.RIBEntry) bool
	RouteEligibleForAdvertisement(device model.Node, route domainroute.RIBEntry) bool
	RouteInstallableInFIB(device model.Node, installed []domainroute.RIBEntry, route domainroute.RIBEntry) bool
}

func (b BaseBehavior) Kind() model.DeviceKind {
	return b.kind
}

func (b BaseBehavior) Profile() model.DeviceProfile {
	return model.ProfileFor(b.kind)
}

func (b BaseBehavior) CheckControlIngress(device model.Node, msg ControlMessage) bool {
	return true
}

func (b BaseBehavior) CheckControlEgress(device model.Node, msg ControlMessage) bool {
	return true
}

func (b BaseBehavior) EvaluateDataACL(device model.Node, pkt PacketMessage, stage string, acls []model.ACL, bindings []model.ACLBinding) PolicyDecision {
...
func (b BaseBehavior) RouteValidForRIB(device model.Node, route domainroute.RIBEntry) bool {
	route = route.Normalize()
	return !route.Attrs.Invalid
}

func (b BaseBehavior) RouteEligibleForAdvertisement(device model.Node, route domainroute.RIBEntry) bool {
	return b.RouteValidForRIB(device, route)
}

func (b BaseBehavior) RouteInstallableInFIB(device model.Node, installed []domainroute.RIBEntry, route domainroute.RIBEntry) bool {
	return b.RouteValidForRIB(device, route)
}
