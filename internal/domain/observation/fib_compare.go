...
func compareFIBEntries(expected, actual []FIBEntry, keyFunc func(FIBEntry) string) Result {
	...
	walkSortedUnion(exp, act, func(key string, e FIBEntry, eok bool, a FIBEntry, aok bool) {
		...
		// Only preference is compared here. RouteMetadata fields (SAFI, TableID,
		// TableName, ProtocolInstance, Age, AgeSeconds, Tag, InstalledReason,
		// Raw) are intentionally excluded from FIB entry comparison to avoid
		// noisy diffs when one source populates them and another does not.
		// They remain available for callers that want to inspect them via the
		// raw structs.
