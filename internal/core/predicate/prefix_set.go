...
type PrefixSet interface {
	...
	ContainsPrefix(prefix netaddr.Prefix) bool
...
func (AnyPrefixSet) ContainsPrefix(prefix netaddr.Prefix) bool {
...
func (s ExactPrefixSet) ContainsPrefix(prefix netaddr.Prefix) bool {
...
func (s PrefixRangeSet) ContainsPrefix(prefix netaddr.Prefix) bool {
...
func (s UnionPrefixSet) ContainsPrefix(prefix netaddr.Prefix) bool {
...
func NewPrefixSet(prefix string, ge, le int) (PrefixSet, error) {
	...
	base, err := netaddr.ParsePrefix(prefix)
	if err != nil {
		return nil, err
	}
	if ge == 0 && le == 0 {
...
func MatchesNLRI(set PrefixSet, prefix netaddr.Prefix) bool {
...
func prefixAddressSpaceOverlaps(set PrefixSet, prefix netaddr.Prefix) bool {
	if prefix.IsZero() {
		return false
	}
	switch s := set.(type) {
	case AnyPrefixSet:
		return true
	case ExactPrefixSet:
		return prefixOverlaps(s.Prefix, prefix)
	case PrefixRangeSet:
		return prefixOverlaps(s.Base, prefix)
	case UnionPrefixSet:
...
func firstPrefixForSet(set PrefixSet) netaddr.Prefix {
	switch s := set.(type) {
	case ExactPrefixSet:
		return s.Prefix
	case PrefixRangeSet:
...
func prefixRangeSetsOverlap(a, b PrefixRangeSet) bool {
	...
	if !prefixOverlaps(a.Base, b.Base) {
		return false
	}
...
func prefixSetContainsPrefixSpace(set PrefixSet, prefix netaddr.Prefix) bool {
...
func prefixOverlaps(a, b netaddr.Prefix) bool {
	return !a.IsZero() && !b.IsZero() && a.Addr().BitLen() == b.Addr().BitLen() && a.Overlaps(b)
}
