...
func TestExactPrefixSetContainsOnlyExactPrefix(t *testing.T) {
	set := ExactPrefixSet{Prefix: netaddr.MustPrefix("10.0.0.0/24")}
	if !set.ContainsPrefix(netaddr.MustPrefix("10.0.0.0/24")) {
		t.Fatalf("exact set should contain identical prefix")
	}
	if set.ContainsPrefix(netaddr.MustPrefix("10.0.0.0/25")) {
...
func TestPrefixRangeSetContainsPrefixByContainmentAndLength(t *testing.T) {
	set := PrefixRangeSet{Base: netaddr.MustPrefix("10.0.0.0/8"), MinLen: 16, MaxLen: 24}
	if !set.ContainsPrefix(netaddr.MustPrefix("10.1.0.0/16")) {
		t.Fatalf("range set should contain permitted child prefix")
	}
	if !set.ContainsPrefix(netaddr.MustPrefix("10.1.2.0/24")) {
		t.Fatalf("range set should contain permitted more-specific prefix")
	}
	if set.ContainsPrefix(netaddr.MustPrefix("10.0.0.0/8")) {
		t.Fatalf("range set should reject prefixes shorter than min length")
	}
	if set.ContainsPrefix(netaddr.MustPrefix("10.1.2.128/25")) {
		t.Fatalf("range set should reject prefixes longer than max length")
	}
	if set.ContainsPrefix(netaddr.MustPrefix("192.0.2.0/24")) {
...
func TestNewPrefixSetBuildsExactAnyAndRangeSets(t *testing.T) {
	...
	if !any.ContainsPrefix(netaddr.MustPrefix("203.0.113.0/24")) {
	...
	if !ranged.ContainsPrefix(netaddr.MustPrefix("10.2.0.0/16")) || ranged.ContainsPrefix(netaddr.MustPrefix("10.2.3.128/25")) {
		t.Fatalf("range set did not enforce ge/le bounds")
	}
}

func TestPrefixSetOverlaps(t *testing.T) {
	a := PrefixRangeSet{Base: netaddr.MustPrefix("10.0.0.0/8"), MinLen: 16, MaxLen: 24}
	b := ExactPrefixSet{Prefix: netaddr.MustPrefix("10.1.0.0/16")}
	c := ExactPrefixSet{Prefix: netaddr.MustPrefix("192.0.2.0/24")}
...
func TestPrefixSetAddressSpaceAndNLRIPredicateOverlapDiffer(t *testing.T) {
	ranged := PrefixRangeSet{Base: netaddr.MustPrefix("10.0.0.0/8"), MinLen: 16, MaxLen: 24}
	base := ExactPrefixSet{Prefix: netaddr.MustPrefix("10.0.0.0/8")}
	packetHost := ExactPrefixSet{Prefix: netaddr.MustPrefix("10.4.1.10/32")}
