...
import (
	...
	"github.com/81ueman/hoyan-lab/internal/core/symbolic"
...
func (c varCond) Eval(ctx Context) bool {
	switch c.kind {
	case condVarNode:
		return !ctx.NodeFailed(topology.NodeID(c.name))
	case condVarLink:
		return !ctx.LinkFailed(topology.LinkID(c.name))
	default:
		return true
	}
}
...
func ExpandLinkVars(c Cond, linksByName map[topology.LinkID]topology.Link) Cond {
	c = normalizeCond(c)
	switch x := c.(type) {
	case varCond:
		...
		link, ok := linksByName[topology.LinkID(x.name)]
		if !ok {
			return x
		}
