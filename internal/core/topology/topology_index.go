...
func (idx *TopologyIndex) ConnectedClass(node string, iface Interface, prefix netaddr.Prefix) ConnectedRouteClass {
	if idx != nil {
		n, ok := idx.Node(node)
		if ok {
...
func (idx *TopologyIndex) ConnectedClassForRoute(node, prefix, iface string) ConnectedRouteClass {
	...
	want, err := netaddr.ParsePrefix(prefix)
	if err != nil {
		return ""
	}
	for _, candidate := range n.Interfaces {
		...
		candidatePrefix := netaddr.PrefixFromNetIP(pfx.Masked())
...
func (idx *TopologyIndex) OriginForIP(addr string) (string, netaddr.Prefix, bool) {
	if idx == nil {
