...
type Node struct {
	...
	Prefixes       []netaddr.Prefix    `yaml:"prefixes"`
...
type ConfiguredRoute struct {
	...
	Prefix          netaddr.Prefix      `yaml:"prefix" json:"prefix"`
...
type PrefixListRule struct {
	...
	Match  predicate.PrefixSet `yaml:"-"`
}
...
type ACLRule struct {
	...
	Match  predicate.PacketSpec `yaml:"-" json:"-"`
	Source ConfigSource         `yaml:"source,omitempty" json:"source,omitempty"`
}
...
func (t *Topology) OriginForIP(addr string) (string, netaddr.Prefix, bool) {
	idx, err := BuildTopologyIndex(t)
	if err != nil {
