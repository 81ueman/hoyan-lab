...
func TestOriginLookupsUseTypedCanonicalPrefixes(t *testing.T) {
	topo := &Topology{Nodes: []Node{
		{Name: "origin", Prefixes: netaddr.MustPrefixes("10.0.0.1/24")},
	}}
...
func TestValidateRejectsInvalidACLFields(t *testing.T) {
	tests := []struct {
		...
		{
			name: "protocol",
			acl:  ACL{Name: "a1", Node: "r1", DefaultAction: ACLDefaultPermit, Rules: []ACLRule{{Seq: 10, Action: ACLDeny, Match: predicate.PacketSpec{Protocol: "gre"}}}},
...
`
	topologyPath := filepath.Join(dir, "lab.clab.yml")
	if err := os.WriteFile(topologyPath, []byte(topo), 0o644); err != nil {
		t.Fatalf("WriteFile(topology) error = %v", err)
	}
	parsedTopology, err := LoadLabTopology(topologyPath)
	if err != nil {
		t.Fatalf("LoadLabTopology() error = %v", err)
	}
	node, ok := parsedTopology.Node("ceos1")
	if !ok {
