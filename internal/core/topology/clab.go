...
func LoadLabTopologyWithOptions(clabPath string, opts LoadLabTopologyOptions) (*Topology, []UnsupportedStatement, error) {
	...
	for _, name := range names {
		...
		if parsedOSPFEnabled(parsed) && parsed.Loopback != "" {
			loopbackPrefix, err := netaddr.ParsePrefix(parsed.Loopback)
			if err != nil {
...
func appendUniquePrefix(prefixes []netaddr.Prefix, prefix netaddr.Prefix) []netaddr.Prefix {
	if prefix.IsZero() {
		return prefixes
	}
...
func parsePrefixes(raw []string) ([]netaddr.Prefix, error) {
	out := make([]netaddr.Prefix, 0, len(raw))
	for _, p := range raw {
		parsed, err := netaddr.ParsePrefix(p)
		if err != nil {
