...
func solveZ3(elements []solver.FailureElement, maxFailures int, goal func(C.Z3_context, map[string]C.Z3_ast) C.Z3_ast, backend string) (solver.Answer, error) {
	cfg := C.Z3_mk_config()
	defer C.Z3_del_config(cfg)
	ctx := C.Z3_mk_context(cfg)
	defer C.Z3_del_context(ctx)
	z3Solver := C.Z3_mk_solver(ctx)
	C.Z3_solver_inc_ref(ctx, z3Solver)
	defer C.Z3_solver_dec_ref(ctx, z3Solver)
	...
	if maxFailures >= 0 {
		C.Z3_solver_assert(ctx, z3Solver, atMost(ctx, vars, elementNames, maxFailures))
	}
	C.Z3_solver_assert(ctx, z3Solver, goal(ctx, vars))
	switch C.Z3_solver_check(ctx, z3Solver) {
	case C.Z3_L_TRUE:
		model := C.Z3_solver_get_model(ctx, z3Solver)
		C.Z3_model_inc_ref(ctx, model)
		defer C.Z3_model_dec_ref(ctx, model)
		var failed []solver.FailureElement
		for _, element := range elements {
			name := element.String()
			var value C.Z3_ast
		...
		return solver.Answer{Sat: true, Failures: failed, Backend: backend}, nil
	case C.Z3_L_FALSE:
		return solver.Answer{Sat: false, Backend: backend}, nil
	default:
		return solver.Answer{Sat: false, Backend: backend}, fmt.Errorf("z3 returned unknown")
	}
}
