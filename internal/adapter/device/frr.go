...
type frrBehavior struct{ device.BaseBehavior }

func NewFRRBehavior() device.DeviceBehavior {
	return frrBehavior{device.NewBaseBehavior(model.KindFRR, bgp.NewFRRDecisionProcess(bgp.DefaultDecisionOptions()))}
}
