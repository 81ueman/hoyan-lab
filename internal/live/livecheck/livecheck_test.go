...
func TestWaitForFRRContainers(t *testing.T) {
	...
	nodes := []topology.Node{{Name: "r1", ContainerName: "clab-test-r1", Kind: "frr"}, {Name: "r2", Kind: "frr"}}
...
func TestWaitForSRLinuxCLIUsesJSONReadinessProbe(t *testing.T) {
	...
	nodes := []topology.Node{
		{Name: "core-gz", ContainerName: "clab-test-core-gz", Kind: topology.KindSRLinux},
		{Name: "r1", ContainerName: "clab-test-r1", Kind: topology.KindFRR},
	}
...
func TestRunSnapshotOfflineDoesNotCollectOrDeploy(t *testing.T) {
	...
	topo, err := topology.LoadLabTopology(topologyPath)
	if err != nil {
		t.Fatalf("LoadLabTopology() error = %v", err)
	}
	nodes := ribcompare.SupportedNodes(topo.Nodes)
	expected := ribcompare.ExpectedForNodes(topo, routing.FromTopology(topo), nodes)
...
func TestRunDataplaneChecksProbesICMPAndTCP(t *testing.T) {
	...
	topo := &topology.Topology{
		Nodes: []topology.Node{
			{Name: "src", ContainerName: "clab-test-src", Kind: topology.KindFRR},
			{Name: "dst", ContainerName: "clab-test-dst", Kind: topology.KindFRR, Prefixes: netaddr.MustPrefixes("10.0.0.10/32")},
		},
		Links: []topology.Link{{Name: "src-dst", A: "src", B: "dst", AIntf: "eth1", BIntf: "eth1", Cost: 1, Subnet: "192.0.2.0/31"}},
	}
	...
	if err := RunDataplaneChecks(context.Background(), runner, topo, routing.FromTopology(topo), queries, ioDiscard{}); err != nil {
...
func TestRunDataplaneChecksFailsOnMismatch(t *testing.T) {
	...
	err := RunDataplaneChecks(context.Background(), runner, topo, routing.FromTopology(topo), queries, ioDiscard{})
...
func TestApplyNftablesPolicies(t *testing.T) {
	topo := &topology.Topology{
		Nodes: []topology.Node{
			{Name: "core-hz", ContainerName: "clab-test-core-hz", Kind: topology.KindFRR},
			{Name: "core-bj", ContainerName: "clab-test-core-bj", Kind: topology.KindFRR},
		},
		ACLs: []topology.ACL{
			{Name: "BLOCK-HTTP-TO-HZ", Node: "core-hz", Source: topology.ConfigSource{Vendor: "nftables"}},
			{Name: "OTHER", Node: "core-bj", Source: topology.ConfigSource{Vendor: "ceos"}},
		},
	}
	runner := &fakeRunner{fn: func(name string, args ...string) ([]byte, error) {
		cmd := name + " " + strings.Join(args, " ")
...
func TestWaitForExpectedRoutesStopsAfterMaxPolls(t *testing.T) {
	...
	nodes := []topology.Node{{Name: "r1", Kind: "frr"}}
...
func TestLinkFailureScenarioInjectsAndCleansBothEndpoints(t *testing.T) {
	topo := &topology.Topology{
		Name: "test-lab",
		Nodes: []topology.Node{
			{Name: "a", Kind: topology.KindFRR, Interfaces: []topology.Interface{{Name: "eth1", Address: "192.0.2.1/24"}}},
			{Name: "b", Kind: topology.KindCEOS, Interfaces: []topology.Interface{{Name: "Ethernet2", Address: "192.0.2.2/24"}}},
		},
		Links: []topology.Link{
...
func TestNodeFailureScenarioStopsNodeAndFiltersActiveFRRNodes(t *testing.T) {
	...
	if got, want := scenario.ActiveNodes, []topology.Node{{Name: "r2", Kind: "frr"}, {Name: "s1", Kind: "srlinux"}}; !reflect.DeepEqual(got, want) {
...
func TestCompareRIBsWithFailuresUsesFailureAwareExpectedRoutes(t *testing.T) {
	topo := &topology.Topology{
		Nodes: []topology.Node{
			{Name: "r1", Kind: "frr", ASN: 65001, Prefixes: netaddr.MustPrefixes("10.0.0.0/24")},
			{Name: "r2", Kind: "frr", ASN: 65002, Prefixes: netaddr.MustPrefixes("10.1.0.0/24")},
		},
	}
	runner := &fakeRunner{fn: func(name string, args ...string) ([]byte, error) {
	...
	err := CompareRIBsWithFailures(context.Background(), runner, topo, RIBFailureScenario{
		...
		ActiveNodes: []topology.Node{{Name: "r2", Kind: "frr", ASN: 65002, Prefixes: netaddr.MustPrefixes("10.1.0.0/24")}},
