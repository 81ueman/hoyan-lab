...
func CompareRIBsWithFailures(ctx context.Context, runner ribcompare.Runner, topo *topology.Topology, scenario RIBFailureScenario, opts RIBFailureCheckOptions) error {
	if opts.Interval == 0 {
	...
	expected := ribcompare.ExpectedForNodesWithFailureSet(topo, routing.FromTopology(topo), activeNodes, scenario.Failures)
...
func LinkFailureScenario(topo *topology.Topology, linkName string) (RIBFailureScenario, error) {
	...
	return RIBFailureScenario{
		Name:     "link-" + link.Name,
		Failures: sim.LinkFailures(topology.LinkID(link.Name)),
...
func linkEndpointClabInterfaces(topo *topology.Topology, link topology.Link) (string, string) {
...
func NodeFailureScenario(topo *topology.Topology, nodeName string) (RIBFailureScenario, error) {
	...
	return RIBFailureScenario{
		Name:        "node-" + nodeName,
		Failures:    sim.NodeFailures(topology.NodeID(nodeName)),
...
func activeSupportedNodes(nodes []topology.Node, failed map[string]bool) []topology.Node {
...
func findLink(topo *topology.Topology, name string) (topology.Link, bool) {
