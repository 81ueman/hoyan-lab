...
type NodeSnapshot struct {
	Kind          topology.DeviceKind             `json:"kind"`
...
func Build(ctx context.Context, topologyPath, lab string, runner ribcompare.Runner, rawDir string, fibOpts fibcompare.Options) (*Snapshot, error) {
	topo, warnings, err := topology.LoadLabTopologyWithOptions(topologyPath, topology.LoadLabTopologyOptions{CollectWarnings: true})
	if err != nil {
		return nil, err
	}
	if lab == "" {
...
func collectRouteTables(ctx context.Context, runner ribcompare.Runner, nodes []topology.Node) ([]ribcompare.NormalizedRoute, error) {
...
func warningStrings(warnings []topology.UnsupportedStatement) []string {
