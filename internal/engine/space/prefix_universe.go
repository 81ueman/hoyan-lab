...
type OverlappingPrefixPredicateError struct {
	Existing  predicate.PrefixSet
	Candidate predicate.PrefixSet
}
...
func CollectPrefixPredicates(topo *topology.Topology, routes routing.TopologyRouting, queries *query.Queries) []predicate.PrefixSet {
	predicates := CollectPrefixPredicateMetadata(topo, routes, queries)
	out := make([]predicate.PrefixSet, 0, len(predicates))
...
func CollectPrefixPredicateMetadata(topo *topology.Topology, routes routing.TopologyRouting, queries *query.Queries) []PrefixPredicate {
	var out []PrefixPredicate
	add := func(source string, kind PrefixPredicateKind, set predicate.PrefixSet) {
		if set == nil {
			return
		}
	...
	if topo != nil {
		for _, node := range topo.Nodes {
			nodeRouting := routes.ForNode(node.Name)
			for _, prefix := range node.Prefixes {
				if !prefix.IsZero() {
					add("route:"+node.Name, PredicateNLRI, predicate.ExactPrefixSet{Prefix: prefix})
				}
			}
			for _, route := range nodeRouting.Routes {
				if !route.Prefix.IsZero() {
					add(fmt.Sprintf("route-source:%s:%s", node.Name, route.Kind), PredicateNLRI, predicate.ExactPrefixSet{Prefix: route.Prefix})
				}
			}
			for _, list := range nodeRouting.PrefixLists {
				for _, rule := range list.Rules {
					if rule.Match != nil {
						add(fmt.Sprintf("prefix-list:%s:%s:%d", node.Name, list.Name, rule.Seq), PredicateNLRI, rule.Match)
						continue
					}
					set, err := predicate.NewPrefixSet(rule.Prefix, rule.Ge, rule.Le)
					if err == nil {
	...
	if queries != nil {
		for _, check := range queries.RouteChecks {
			if !check.Prefix.IsZero() {
				add("query-route:"+check.Name, PredicateNLRI, predicate.ExactPrefixSet{Prefix: check.Prefix})
			}
		}
		...
		for _, check := range queries.FailureChecks {
			if !check.Prefix.IsZero() {
				add("query-failure:"+check.Name, PredicateAddressSpace, predicate.ExactPrefixSet{Prefix: check.Prefix})
				continue
			}
			for _, set := range destinationPrefixSets(topo, check.To) {
...
func BuildPrefixUniverse(predicates []predicate.PrefixSet) (PrefixUniverse, error) {
...
func NewPrefixUniverse(topo *topology.Topology, routes routing.TopologyRouting, queries *query.Queries) (PrefixUniverse, error) {
	return BuildPrefixUniverseFromPredicates(CollectPrefixPredicateMetadata(topo, routes, queries))
}

func (u PrefixUniverse) ClassForPrefix(prefix netaddr.Prefix) (PrefixClassID, bool) {
...
func prefixSetContainsPrefixSpace(set predicate.PrefixSet, prefix netaddr.Prefix) bool {
	if set == nil || prefix.IsZero() {
		return false
	}
	switch s := set.(type) {
	case predicate.AnyPrefixSet:
		return true
...
func (u PrefixUniverse) ClassesMatching(set predicate.PrefixSet) []PrefixClassID {
	if set == nil {
		return nil
	}
	var out []PrefixClassID
	for _, class := range u.Classes {
		if predicate.AddressSpaceOverlaps(class.Space, set) {
...
func destinationPrefixSets(topo *topology.Topology, destination string) []predicate.PrefixSet {
	...
	out := make([]predicate.PrefixSet, 0, len(node.Prefixes))
	for _, prefix := range node.Prefixes {
		if !prefix.IsZero() {
			out = append(out, predicate.ExactPrefixSet{Prefix: prefix})
		}
	}
	return out
}
...
func prefixSetCIDRCount(set predicate.PrefixSet) int {
	switch s := set.(type) {
	case nil:
		return 0
	case predicate.UnionPrefixSet:
...
func prefixSetIPv4Intervals(set predicate.PrefixSet) ([]prefixInterval, error) {
	switch s := set.(type) {
	case predicate.AnyPrefixSet:
...
func prefixIPv4Interval(prefix netaddr.Prefix) ([]prefixInterval, error) {
	if prefix.IsZero() {
		return nil, nil
	}
	addr := prefix.Addr()
...
func prefixClassMatchesPredicate(spaceSet predicate.PrefixSet, pred PrefixPredicate) bool {
	switch normalizedPrefixPredicateKind(pred.Kind) {
	case PredicateNLRI:
		return prefixSpaceMatchesNLRIPredicate(spaceSet, pred.Set)
	default:
		return predicate.AddressSpaceOverlaps(spaceSet, pred.Set)
	}
}

func prefixSpaceMatchesNLRIPredicate(space, pred predicate.PrefixSet) bool {
	if space == nil || pred == nil {
		return false
	}
	switch s := space.(type) {
	case predicate.AnyPrefixSet:
		return predicate.NLRIPredicateOverlaps(predicate.AnyPrefixSet{}, pred)
	case predicate.ExactPrefixSet:
		return prefixMatchesNLRIPredicateSpace(s.Prefix, pred)
	case predicate.PrefixRangeSet:
		return prefixMatchesNLRIPredicateSpace(s.Base, pred)
	case predicate.UnionPrefixSet:
		for _, child := range s.Sets {
			if prefixSpaceMatchesNLRIPredicate(child, pred) {
				return true
			}
		}
		return false
	default:
		return predicate.NLRIPredicateOverlaps(space, pred)
	}
}

func prefixMatchesNLRIPredicateSpace(space netaddr.Prefix, pred predicate.PrefixSet) bool {
	if space.IsZero() {
		return false
	}
	switch p := pred.(type) {
	...
	case predicate.UnionPrefixSet:
		...
		return false
	default:
		return pred.ContainsPrefix(space)
	}
}

func prefixSetForInterval(interval prefixInterval) predicate.PrefixSet {
	prefixes := prefixesForIPv4Range(interval.lo, interval.hi)
	sets := make([]predicate.PrefixSet, 0, len(prefixes))
	for _, prefix := range prefixes {
		sets = append(sets, predicate.ExactPrefixSet{Prefix: prefix})
	}
	if len(sets) == 1 {
		return sets[0]
	}
	return predicate.UnionPrefixSet{Sets: sets}
}

func prefixesForIPv4Range(lo, hi uint32) []netaddr.Prefix {
	...
	for current := uint64(lo); current <= uint64(hi); {
		...
		out = append(out, netaddr.PrefixFromNetIP(netip.PrefixFrom(uint32ToIPv4(uint32(current)), prefixLen)))
		current += size
	}
	return out
}
