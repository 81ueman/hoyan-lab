...
func TestSymbolicUnreachableReasonsUseSelectedCandidateCondition(t *testing.T) {
	...
	if primaryNodeReason.Cond.Eval(engine.FailureContext(failure.NewSet([]topology.LinkID{"prefer-primary"}, []topology.NodeID{"primary"}))) {
...
func TestRecursiveNextHopReasonsMatchConcretePacketReachable(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			entry: FIBEntry{
				Prefix:         netaddr.MustPrefix("10.0.0.0/24").NetIP(),
...
func selectedDiscardCandidateEngine() *Engine {
	...
	return NewEngine(idx, nil, testFIB(map[string][]FIBEntry{
		"src": {
			{Prefix: pfx.NetIP(), SourceKind: topology.RouteSourceBlackhole, Discard: true, Condition: failure.LinkVar("prefer-discard")},
...
func selectedProblemCandidateEngine(problemNextHop string) *Engine {
	...
	if problemNextHop != "" {
		nodes = append(nodes, topology.Node{Name: problemNextHop, Kind: topology.KindFRR})
	}
	idx := mustTopologyIndex(&topology.Topology{
		Nodes: nodes,
		Links: []topology.Link{
...
func recursiveProblemEngine(entry FIBEntry) *Engine {
	...
	idx := mustTopologyIndex(&topology.Topology{
		Nodes: []topology.Node{
			{Name: "src", Kind: topology.KindFRR},
			{Name: "dst", Kind: topology.KindFRR, Prefixes: []netaddr.Prefix{pfx}},
			{Name: "orphan", Kind: topology.KindFRR},
		},
		Links: []topology.Link{},
	})
	return NewEngine(idx, nil, testFIB(map[string][]FIBEntry{
