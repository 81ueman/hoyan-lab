...
func TestRouteReachabilityParityMatrix(t *testing.T) {
	...
	AssertSymbolicConcreteRouteParity(t, engine, "src", "10.0.0.0/24", []failure.Set{
		...
		failure.Nodes("backup"),
		failure.Nodes("primary", "backup"),
		failure.NewSet([]topology.LinkID{"src-primary"}, []topology.NodeID{"backup"}),
	})
}
...
func routeRedundantPathEngine() *Engine {
	...
	rib := map[string]map[string]map[string][]controlplane.RIBEntry{
		"src": {
			string(topology.NetworkInstanceDefault): {pfx.String(): {
				{
...
func longestPrefixFallbackEngine() *Engine {
	dstPfx := netaddr.MustPrefix("10.0.0.0/16")
	specificPfx := netaddr.MustPrefix("10.0.0.0/24")
	idx := mustTopologyIndex(&topology.Topology{
		Nodes: []topology.Node{
			{Name: "src", Kind: topology.KindFRR},
			{Name: "specific", Kind: topology.KindFRR},
			{Name: "fallback", Kind: topology.KindFRR},
			{Name: "dst", Kind: topology.KindFRR, Prefixes: []netaddr.Prefix{dstPfx}},
		},
		Links: []topology.Link{
...
func samePrefixCandidatesEngine() *Engine {
	...
	idx := mustTopologyIndex(&topology.Topology{
		Nodes: []topology.Node{
			{Name: "src", Kind: topology.DeviceKind("generic")},
			{Name: "primary", Kind: topology.KindFRR},
			{Name: "backup", Kind: topology.KindFRR},
			{Name: "dst", Kind: topology.KindFRR, Prefixes: []netaddr.Prefix{pfx}},
		},
...
func noRouteEngine() *Engine {
	...
	idx := mustTopologyIndex(&topology.Topology{
		...
		Links: []topology.Link{{Name: "src-dst", A: "src", B: "dst", Cost: 1}},
	})
	return NewEngine(idx, nil, testFIB(map[string][]FIBEntry{}))
}
...
func discardRouteEngine(cond failure.Cond) *Engine {
	...
	return NewEngine(idx, nil, testFIB(map[string][]FIBEntry{
		"src": {{Prefix: pfx.NetIP(), SourceKind: topology.RouteSourceBlackhole, Discard: true, Condition: cond}},
	}))
}
...
func ingressACLDenyEngine() *Engine {
	...
	idx := mustTopologyIndex(&topology.Topology{
		...
		ACLs: testACLs("DENY-TCP-IN", "mid", topology.ACLRule{
...
func egressACLDenyEngine(policyInterface, policyProtocol string) *Engine {
	...
	idx := mustTopologyIndex(&topology.Topology{
		...
		ACLs: testACLs("DENY-POLICY", "mid", topology.ACLRule{
			Seq: 10, Action: topology.ACLDeny, Match: predicate.PacketSpec{Protocol: policyProtocol, DstSet: predicate.ExactPrefixSet{Prefix: pfx}},
		}),
...
func mustTopologyIndex(topo *topology.Topology) *topology.TopologyIndex {
