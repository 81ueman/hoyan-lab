...
func (e *Engine) SymbolicLookupFIB(node, dst string) []SymbolicFIBCandidate {
	return e.SymbolicLookupFIBVRF(node, string(topology.NetworkInstanceDefault), dst)
}
...
func (e *Engine) SymbolicLookupFIBVRF(node, vrf, dst string) []SymbolicFIBCandidate {
	...
	entries := matchingFIBEntries(e.fib[node][string(topology.NormalizeNetworkInstance(vrf))], ip)
	return e.symbolicLookupFIBEntries(entries)
}

func (e *Engine) SymbolicLookupFIBForPrefixSet(node string, dst predicate.PrefixSet) []SymbolicFIBCandidate {
	return e.SymbolicLookupFIBForPrefixSetVRF(node, string(topology.NetworkInstanceDefault), dst)
}

func (e *Engine) SymbolicLookupFIBForPrefixSetVRF(node, vrf string, dst predicate.PrefixSet) []SymbolicFIBCandidate {
	entries := matchingFIBEntriesForPrefixSet(e.fib[node][string(topology.NormalizeNetworkInstance(vrf))], dst)
	return e.symbolicLookupFIBEntries(entries)
}
...
func (e *Engine) SymbolicRouteReachability(from, prefix string) SymbolicRouteReachabilityResult {
	return e.SymbolicRouteReachabilityVRF(from, string(topology.NetworkInstanceDefault), prefix)
}
...
func (e *Engine) SymbolicRouteReachabilityVRF(from, vrf, prefix string) SymbolicRouteReachabilityResult {
	...
	routes := e.rib[from][string(topology.NormalizeNetworkInstance(vrf))][pfx.String()]
...
func (e *Engine) SymbolicRouteReachabilityForPrefixSet(from string, dst predicate.PrefixSet) SymbolicRouteReachabilityResult {
	return e.SymbolicRouteReachabilityForPrefixSetVRF(from, string(topology.NetworkInstanceDefault), dst)
}

func (e *Engine) SymbolicRouteReachabilityForPrefixSetVRF(from, vrf string, dst predicate.PrefixSet) SymbolicRouteReachabilityResult {
	reachable := failure.False()
	result := SymbolicRouteReachabilityResult{Reachable: reachable, Unreachable: failure.True()}
	if e == nil || e.idx == nil {
		result.Reason = "topology index is unavailable"
...
func (e *Engine) SymbolicRouteReachabilityForClass(from string, universe space.PrefixUniverse, classID space.PrefixClassID) SymbolicRouteReachabilityResult {
	return e.SymbolicRouteReachabilityForClassVRF(from, string(topology.NetworkInstanceDefault), universe, classID)
}

func (e *Engine) SymbolicRouteReachabilityForClassVRF(from, vrf string, universe space.PrefixUniverse, classID space.PrefixClassID) SymbolicRouteReachabilityResult {
...
func (e *Engine) SymbolicPacketReachability(from, to, protocol string) SymbolicReachabilityResult {
	return e.SymbolicPacketReachabilitySpec(from, to, predicate.PacketSpec{Protocol: protocol})
}

func (e *Engine) SymbolicPacketReachabilitySpec(from, to string, spec predicate.PacketSpec) SymbolicReachabilityResult {
	return e.SymbolicPacketReachabilitySpecVRF(from, string(topology.NetworkInstanceDefault), to, spec)
}

func (e *Engine) SymbolicPacketReachabilitySpecVRF(from, vrf, to string, spec predicate.PacketSpec) SymbolicReachabilityResult {
	reachable := failure.False()
	...
	dstSet := predicate.ExactPrefixSet{Prefix: dstPrefix}
	return e.symbolicPacketReachabilityForPrefixSet(from, string(topology.NormalizeNetworkInstance(vrf)), dstSet, dstPrefix.NetIP(), spec, failure.And(failure.NodeVar(from), failure.NodeVar(dstNode)))
}

func (e *Engine) SymbolicPacketReachabilityForPrefixSet(from string, dst predicate.PrefixSet, protocol string) SymbolicReachabilityResult {
	return e.SymbolicPacketReachabilityForPrefixSetSpec(from, dst, predicate.PacketSpec{Protocol: protocol})
}

func (e *Engine) SymbolicPacketReachabilityForPrefixSetSpec(from string, dst predicate.PrefixSet, spec predicate.PacketSpec) SymbolicReachabilityResult {
	return e.SymbolicPacketReachabilityForPrefixSetSpecVRF(from, string(topology.NetworkInstanceDefault), dst, spec)
}

func (e *Engine) SymbolicPacketReachabilityForPrefixSetSpecVRF(from, vrf string, dst predicate.PrefixSet, spec predicate.PacketSpec) SymbolicReachabilityResult {
	...
	return e.symbolicPacketReachabilityForPrefixSet(from, string(topology.NormalizeNetworkInstance(vrf)), dst, rep.NetIP(), spec, failure.NodeVar(from))
}

func (e *Engine) SymbolicPacketReachabilityForClass(from string, universe space.PrefixUniverse, classID space.PrefixClassID, protocol string) SymbolicReachabilityResult {
	return e.SymbolicPacketReachabilityForClassVRF(from, string(topology.NetworkInstanceDefault), universe, classID, protocol)
}

func (e *Engine) SymbolicPacketReachabilityForClassVRF(from, vrf string, universe space.PrefixUniverse, classID space.PrefixClassID, protocol string) SymbolicReachabilityResult {
	for _, class := range universe.Classes {
		if class.ID == classID {
			return e.SymbolicPacketReachabilityForPrefixSetSpecVRF(from, vrf, class.Space, predicate.PacketSpec{Protocol: protocol})
		}
	}
...
func (e *Engine) SymbolicPacketReachabilityForPacketClass(from string, class space.PacketClass) SymbolicReachabilityResult {
	if class.DstSet == nil {
		return SymbolicReachabilityResult{
			Reachable:   failure.False(),
			Unreachable: failure.True(),
...
func (e *Engine) symbolicPacketReachabilityForPrefixSet(from, vrf string, dst predicate.PrefixSet, packetPrefix netip.Prefix, spec predicate.PacketSpec, initialCond failure.Cond) SymbolicReachabilityResult {
...
func routePath(idx *topology.TopologyIndex, route controlplane.RIBEntry) Path {
	route = route.Normalize()
...
func (e *Engine) symbolicForward(state SymbolicPacketState, vrf string, dst predicate.PrefixSet, packetPrefix netip.Prefix, maxHops int, visited map[string]bool, states []SymbolicPacketState, paths *[]SymbolicPacketPath, blocked *[]SymbolicPacketBlockedPath, reasons *[]SymbolicUnreachableReason) {
...
func matchingFIBEntriesForPrefixSet(entries []FIBEntry, dst predicate.PrefixSet) []FIBEntry {
	if dst == nil {
		return nil
	}
	var out []FIBEntry
	for _, entry := range entries {
		entrySet := predicate.ExactPrefixSet{Prefix: netaddr.PrefixFromNetIP(entry.Prefix)}
		if predicate.AddressSpaceOverlaps(entrySet, dst) {
			out = append(out, entry)
		}
	}
	sort.SliceStable(out, func(i, j int) bool {
...
func representativePrefixForSet(set predicate.PrefixSet) (netaddr.Prefix, bool) {
	switch s := set.(type) {
	case predicate.ExactPrefixSet:
		return s.Prefix, !s.Prefix.IsZero()
	case predicate.PrefixRangeSet:
...
func (e *Engine) originNodesForPrefixSetVRF(vrf string, dst predicate.PrefixSet) []string {
...
func (e *Engine) linkUpCond(link topology.Link) failure.Cond {
...
func ingressInterface(link topology.Link, node string) string {
	switch node {
	case link.A:
		return link.AIntf
	case link.B:
