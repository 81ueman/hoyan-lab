...
func testFIB(raw map[string][]FIBEntry) map[string]map[string][]FIBEntry {
	...
	for node, entries := range raw {
		out[node] = map[string][]FIBEntry{string(topology.NetworkInstanceDefault): entries}
	}
	return out
}
...
func TestRouteReachableUsesSelectedCondition(t *testing.T) {
	idx, err := topology.BuildTopologyIndex(&topology.Topology{
		Nodes: []topology.Node{
			{Name: "a", Kind: topology.KindFRR},
			{Name: "b", Kind: topology.KindFRR, Prefixes: []netaddr.Prefix{netaddr.MustPrefix("10.0.0.0/24")}},
		},
		Links: []topology.Link{{Name: "a-b", A: "a", B: "b", Cost: 10}},
	})
	if err != nil {
		t.Fatal(err)
	}
	rib := map[string]map[string]map[string][]controlplane.RIBEntry{
		"a": {string(topology.NetworkInstanceDefault): {"10.0.0.0/24": {{
			NLRI:              controlplane.RouteNLRI{Prefix: netaddr.MustPrefix("10.0.0.0/24")},
			Provenance:        controlplane.RouteProvenance{PathNodes: []string{"b", "a"}, PathLinks: []string{"a-b"}},
			SelectedCond:      failure.LinkVar("a-b"),
			RouteSource:       topology.ConfiguredRoute{NetworkInstance: topology.NetworkInstanceDefault},
...
func TestSymbolicPacketReachabilitySinglePathIncludesForwardingConditions(t *testing.T) {
	...
	idx, err := topology.BuildTopologyIndex(&topology.Topology{
		Nodes: []topology.Node{
			{Name: "src", Kind: topology.KindFRR},
			{Name: "mid", Kind: topology.KindFRR},
			{Name: "dst", Kind: topology.KindFRR, Prefixes: []netaddr.Prefix{pfx}},
		},
		Links: []topology.Link{
	...
	for _, failed := range []topology.LinkID{"src-mid", "mid-dst"} {
	...
	for _, failed := range []topology.NodeID{"src", "mid", "dst"} {
...
func TestSymbolicPacketReachabilityForExactPrefixSetMatchesConcreteIP(t *testing.T) {
	...
	prefixSet := e.SymbolicPacketReachabilityForPrefixSet("src", predicate.ExactPrefixSet{Prefix: pfx}, "icmp")
	...
	universe, err := space.BuildPrefixUniverse([]predicate.PrefixSet{predicate.ExactPrefixSet{Prefix: pfx}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestSymbolicPacketReachabilityRedundantPathsAreORed(t *testing.T) {
	...
	idx, err := topology.BuildTopologyIndex(&topology.Topology{
		Nodes: []topology.Node{
			{Name: "src", Kind: topology.KindFRR},
			{Name: "primary", Kind: topology.KindFRR},
			{Name: "backup", Kind: topology.KindFRR},
			{Name: "dst", Kind: topology.KindFRR, Prefixes: []netaddr.Prefix{pfx}},
		},
		Links: []topology.Link{
...
func TestSymbolicLookupFIBAddsNotHigherMatchingConditions(t *testing.T) {
	pfx := netaddr.MustPrefix("10.0.0.0/24")
	defaultPfx := netaddr.MustPrefix("0.0.0.0/0")
	idx, err := topology.BuildTopologyIndex(&topology.Topology{
		Nodes: []topology.Node{
			{Name: "src", Kind: topology.KindFRR},
			{Name: "specific", Kind: topology.KindFRR},
			{Name: "fallback", Kind: topology.KindFRR},
		},
		Links: []topology.Link{
...
func TestSymbolicLookupFIBForPrefixSetAddsNotHigherMatchingConditions(t *testing.T) {
	...
	candidates := e.SymbolicLookupFIBForPrefixSet("src", predicate.ExactPrefixSet{Prefix: pfx})
	if len(candidates) != 2 {
...
func TestPacketReachableMatchesPolicyInterface(t *testing.T) {
	...
	idx, err := topology.BuildTopologyIndex(&topology.Topology{
		...
		ACLs: testACLs("NFT-DENY", "mid", topology.ACLRule{
	...
	fib := testFIB(map[string][]FIBEntry{
		"src": {{Prefix: pfx.NetIP(), NextHop: "mid", Condition: failure.True()}},
		"mid": {{Prefix: pfx.NetIP(), NextHop: "dst", Condition: failure.True()}},
	})
	e := NewEngineWithRouting(idx, routes, nil, fib)
	...
	routes.ACLBindings[0].Interface = "eth9"
	e = NewEngineWithRouting(idx, routes, nil, fib)
...
func TestPacketReachableMatchesPolicyDstPort(t *testing.T) {
	...
	idx, err := topology.BuildTopologyIndex(&topology.Topology{
		...
		ACLs: testACLs("DENY-HTTP", "mid", topology.ACLRule{
			Seq: 10, Action: topology.ACLDeny, Match: predicate.PacketSpec{Protocol: "tcp", DstSet: predicate.ExactPrefixSet{Prefix: pfx}, DstPort: predicate.ExactPortSet{Port: 80}},
		}),
	...
	http := predicate.PacketSpec{Protocol: "tcp", DstPort: predicate.ExactPortSet{Port: 80}}
	...
	https := predicate.PacketSpec{Protocol: "tcp", DstPort: predicate.ExactPortSet{Port: 443}}
	...
	if _, ok, reason := e.PacketReachableSpec("src", "10.0.0.10", predicate.PacketSpec{Protocol: "icmp"}, failure.None()); !ok {
...
func TestPacketReachableUsesACLPermitAndDefaultAction(t *testing.T) {
	...
	idx, err := topology.BuildTopologyIndex(&topology.Topology{
		Nodes: []topology.Node{
			{Name: "src", Kind: topology.KindFRR},
			{Name: "mid", Kind: topology.KindCEOS},
			{Name: "dst", Kind: topology.KindFRR, Prefixes: []netaddr.Prefix{pfx}},
		},
		Links: []topology.Link{
		...
		ACLs: []topology.ACL{{
			...
			Rules: []topology.ACLRule{
				{Seq: 10, Action: topology.ACLPermit, Match: predicate.PacketSpec{Protocol: "tcp", DstSet: predicate.ExactPrefixSet{Prefix: pfx}, DstPort: predicate.ExactPort(443)}},
				{Seq: 20, Action: topology.ACLDeny, Match: predicate.PacketSpec{Protocol: "tcp", DstSet: predicate.ExactPrefixSet{Prefix: pfx}, DstPort: predicate.ExactPort(80)}},
			},
		}},
		ACLBindings: []topology.ACLBinding{{Node: "mid", Interface: "Ethernet2", Direction: "egress", ACLName: "WEB"}},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	https := predicate.PacketSpec{Protocol: "tcp", DstPort: predicate.ExactPort(443)}
	if _, ok, reason := e.PacketReachableSpec("src", "10.0.0.10", https, failure.None()); !ok {
		t.Fatalf("tcp/443 PacketReachableSpec() ok=false reason=%q, want ACL permit", reason)
	}
	http := predicate.PacketSpec{Protocol: "tcp", DstPort: predicate.ExactPort(80)}
	...
	ssh := predicate.PacketSpec{Protocol: "tcp", DstPort: predicate.ExactPort(22)}
...
func TestSymbolicPacketReachabilityForClassAppliesDstPrefixPolicy(t *testing.T) {
	...
	idx, err := topology.BuildTopologyIndex(&topology.Topology{
		...
		ACLs: testACLs("deny-tcp", "mid", topology.ACLRule{
			Seq: 10, Action: topology.ACLDeny, Match: predicate.PacketSpec{Protocol: "tcp", DstSet: predicate.ExactPrefixSet{Prefix: pfx}},
		}),
...
func TestSymbolicPacketReachabilityRecordsPolicyDeny(t *testing.T) {
	...
	idx, err := topology.BuildTopologyIndex(&topology.Topology{
		...
		ACLs: testACLs("NFT-DENY-TCP", "mid", topology.ACLRule{
			Seq:    10,
			Action: topology.ACLDeny,
			Match:  predicate.PacketSpec{Protocol: "tcp", DstSet: predicate.ExactPrefixSet{Prefix: pfx}},
...
func TestSymbolicPacketReachabilityDoesNotExploreLoopsForever(t *testing.T) {
	...
	idx, err := topology.BuildTopologyIndex(&topology.Topology{
		...
		Links: []topology.Link{{Name: "a-b", A: "a", B: "b", Cost: 1}},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func testACLs(name, node string, rules ...topology.ACLRule) []topology.ACL {
	return []topology.ACL{{
		Name:          name,
		Node:          node,
		Vendor:        topology.KindFRR,
		DefaultAction: topology.ACLDefaultPermit,
		Rules:         rules,
	}}
}

func testACLBindings(name, node, iface, direction string) []topology.ACLBinding {
	return []topology.ACLBinding{{Node: node, Interface: iface, Direction: direction, ACLName: name}}
}
...
func TestDeriveFIBUsesVendorInstallEligibility(t *testing.T) {
	...
	equivalentRoutes := []controlplane.RIBEntry{
		{NLRI: controlplane.RouteNLRI{Prefix: prefix}, Provenance: controlplane.RouteProvenance{OriginNode: "a", PathNodes: []string{"a", "rx"}}, Attrs: controlplane.BGPAttributes{LocalPref: 100, ASPath: []uint32{65100}}, SelectedCond: failure.LinkVar("path-a"), RouteSource: topology.ConfiguredRoute{NetworkInstance: topology.NetworkInstanceDefault}},
		{NLRI: controlplane.RouteNLRI{Prefix: prefix}, Provenance: controlplane.RouteProvenance{OriginNode: "b", PathNodes: []string{"b", "rx"}}, Attrs: controlplane.BGPAttributes{LocalPref: 100, ASPath: []uint32{65200}}, SelectedCond: failure.LinkVar("path-b"), RouteSource: topology.ConfiguredRoute{NetworkInstance: topology.NetworkInstanceDefault}},
	}

	frrIdx, err := topology.BuildTopologyIndex(&topology.Topology{Nodes: []topology.Node{{Name: "rx", Kind: topology.KindFRR, ASN: 65000}}})
	if err != nil {
		t.Fatal(err)
	}
	frrRIB := map[string]map[string]map[string][]controlplane.RIBEntry{"rx": {string(topology.NetworkInstanceDefault): {prefix.String(): append([]controlplane.RIBEntry(nil), equivalentRoutes...)}}}
	frrFIB := map[string]map[string][]FIBEntry{}
	NewEngine(frrIdx, frrRIB, frrFIB).DeriveFIB()
	if got := len(frrFIB["rx"][string(topology.NetworkInstanceDefault)]); got != 1 {
		t.Fatalf("FRR FIB entries = %d, want equivalent route collapsed to 1", got)
	}

	genericKind := topology.DeviceKind("generic")
	genericIdx, err := topology.BuildTopologyIndex(&topology.Topology{Nodes: []topology.Node{{Name: "rx", Kind: genericKind, ASN: 65000}}})
	if err != nil {
		t.Fatal(err)
	}
	genericRIB := map[string]map[string]map[string][]controlplane.RIBEntry{"rx": {string(topology.NetworkInstanceDefault): {prefix.String(): append([]controlplane.RIBEntry(nil), equivalentRoutes...)}}}
	genericFIB := map[string]map[string][]FIBEntry{}
	NewEngine(genericIdx, genericRIB, genericFIB).DeriveFIB()
	if got := len(genericFIB["rx"][string(topology.NetworkInstanceDefault)]); got != 2 {
		t.Fatalf("generic FIB entries = %d, want equivalent routes kept", got)
	}
	genericEntries := genericFIB["rx"][string(topology.NetworkInstanceDefault)]
...
func TestDeriveFIBMarksAddressOnlyNextHopUnresolved(t *testing.T) {
	prefix := netaddr.MustPrefix("10.0.0.0/24")
	idx, err := topology.BuildTopologyIndex(&topology.Topology{
		Nodes: []topology.Node{{Name: "rx", Kind: topology.KindFRR}},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	NewEngine(idx, map[string]map[string]map[string][]controlplane.RIBEntry{
		"rx": {string(topology.NetworkInstanceDefault): {prefix.String(): {{
	...
	if got := len(fib["rx"][string(topology.NetworkInstanceDefault)]); got != 1 {
		t.Fatalf("FIB entries = %d, want 1", got)
	}
	entry := fib["rx"][string(topology.NetworkInstanceDefault)][0]
...
func TestDeriveFIBMarksBlackholeRouteAsDiscard(t *testing.T) {
	prefix := netaddr.MustPrefix("10.0.0.0/24")
	idx := mustTopologyIndex(&topology.Topology{
		Nodes: []topology.Node{{Name: "src", Kind: topology.KindFRR}},
	})
	rib := map[string]map[string]map[string][]controlplane.RIBEntry{
		"src": {
			string(topology.NetworkInstanceDefault): {prefix.String(): {
				{
					NLRI:         controlplane.RouteNLRI{Prefix: prefix},
					SourceKind:   topology.RouteSourceBlackhole,
					RouteSource:  topology.ConfiguredRoute{Prefix: prefix, Kind: topology.RouteSourceBlackhole, Interface: "Null0"},
					SelectedCond: failure.True(),
				},
				{
					NLRI:              controlplane.RouteNLRI{Prefix: prefix},
	...
	if len(fib["src"][string(topology.NetworkInstanceDefault)]) != 1 {
		t.Fatalf("FIB entries = %#v, want local blackhole selected over same-prefix BGP", fib["src"])
	}
	entry := fib["src"][string(topology.NetworkInstanceDefault)][0]
	if !entry.Discard || entry.SourceKind != topology.RouteSourceBlackhole || entry.Interface != "Null0" {
