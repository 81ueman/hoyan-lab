...
func (frrCollector) CollectRouteTables(ctx context.Context, runner Runner, nodes []topology.Node) ([]NormalizedBgpRoute, error) {
...
func (ceosCollector) CollectRouteTables(ctx context.Context, runner Runner, nodes []topology.Node) ([]NormalizedBgpRoute, error) {
	var out []NormalizedBgpRoute
	for _, n := range nodes {
		containerName := n.RuntimeName()
...
func (srlinuxCollector) CollectRouteTables(ctx context.Context, runner Runner, nodes []topology.Node) ([]NormalizedBgpRoute, error) {
	var out []NormalizedBgpRoute
	for _, n := range nodes {
		containerName := n.RuntimeName()
...
func normalizeSRLinuxStaticRouteNextHops(node topology.Node, routes []NormalizedBgpRoute) {
	configured := map[string]string{}
	for _, route := range node.Routes {
		if route.Kind != topology.RouteSourceStatic || route.NextHop == "" {
			continue
		}
		vrf := string(topology.NormalizeNetworkInstance(string(route.NetworkInstance)))
