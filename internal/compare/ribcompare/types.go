...
type BgpRibCollector interface {
	Collect(ctx context.Context, runner Runner, nodes []topology.Node) ([]NormalizedBgpRoute, error)
}

type RouteTableCollector interface {
	CollectRouteTables(ctx context.Context, runner Runner, nodes []topology.Node) ([]NormalizedBgpRoute, error)
}
