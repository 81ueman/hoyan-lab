...
func TestExpectedRoutesIncludesMultipleBgpPaths(t *testing.T) {
	...
	routes := Expected(topo, routing.FromTopology(topo))
	if len(routes) == 0 {
...
func TestExpectedRoutesIncludesStaticAndConnectedSources(t *testing.T) {
	...
	blackholePrefix := netaddr.MustPrefix("198.51.100.0/24")
	topo := &topology.Topology{
		Nodes: []topology.Node{{
			...
			Routes: []topology.ConfiguredRoute{
				{Prefix: staticPrefix, Kind: topology.RouteSourceStatic, NextHop: "192.0.2.2"},
				{Prefix: blackholePrefix, Kind: topology.RouteSourceBlackhole, Interface: "Null0"},
			},
		}, {
			Name:       "r2",
			Kind:       topology.KindFRR,
		...
		Links: []topology.Link{{Name: "r1-r2", A: "r1", B: "r2", AIntf: "eth1", BIntf: "eth1", Cost: 1}},
	}
	routes := Expected(topo, routing.FromTopology(topo))
...
func TestExpectedConnectedRoutesCarryClassAndIncludeLoopbackService(t *testing.T) {
	topo := &topology.Topology{
		...
		Links: []topology.Link{{Name: "svc-r2", A: "svc", B: "r2", AIntf: "eth1", BIntf: "eth1", Cost: 1}},
	}
	routes := Expected(topo, routing.FromTopology(topo))
	link := routeByPrefixProtocol(routes, "192.0.2.0/31", "connected")
	if link == nil || link.ConnectedClass != topology.ConnectedRouteClassLink {
		t.Fatalf("link connected route = %#v", link)
	}
	service := routeByPrefixProtocol(routes, "10.0.0.10/32", "connected")
	if service == nil || service.ConnectedClass != topology.ConnectedRouteClassService {
...
func TestExpectedOSPFSuppressesNonFRRLocalRoutes(t *testing.T) {
	...
	topo.Nodes[1].Kind = topology.KindCEOS
	routes := Expected(topo, routing.FromTopology(topo))
...
func ospfExpectedNode(name, loopback string, ifaces map[string]string, costs map[string]int) topology.Node {
	interfaces := []topology.Interface{{Name: "lo", Address: loopback}}
	ospfIfaces := map[string]topology.OSPFInterface{"lo": {Name: "lo", Area: "0", Passive: true}}
...
func ospfExpectedAreaNode(name, loopback string, ifaces map[string]string, areas map[string]string) topology.Node {
	interfaces := []topology.Interface{{Name: "lo", Address: loopback}}
	ospfIfaces := map[string]topology.OSPFInterface{"lo": {Name: "lo", Area: areas["lo"], Passive: true}}
	networks := []topology.OSPFNetwork{{Prefix: netaddr.MustPrefix(loopback), Area: areas["lo"]}}
	for name, addr := range ifaces {
		interfaces = append(interfaces, topology.Interface{Name: name, Address: addr})
		ospfIfaces[name] = topology.OSPFInterface{Name: name, Area: areas[name], Cost: 1}
		networks = append(networks, topology.OSPFNetwork{Prefix: netaddr.MustPrefix(addr), Area: areas[name]})
	}
	return topology.Node{
		Name:       name,
		Kind:       topology.KindFRR,
...
func TestCollectIncludesInstalledStaticAndConnectedRoutes(t *testing.T) {
	...
	routes, err := Collect(context.Background(), runner, []topology.Node{{Name: "r1", Kind: topology.KindFRR, ContainerName: "r1"}})
	if err != nil {
...
func TestCollectSkipsBGPCommandsForNodesWithoutASN(t *testing.T) {
	...
	routes, err := Collect(context.Background(), runner, []topology.Node{{Name: "ceos", Kind: topology.KindCEOS, ContainerName: "ceos1"}})
	if err != nil {
		t.Fatalf("Collect() error = %v", err)
	}
...
func TestExpectedPathUsesModeledAttributes(t *testing.T) {
	...
	node := topology.Node{Name: "r1", Kind: "frr"}
...
func TestExpectedPathUsesDeviceBehaviorValidity(t *testing.T) {
	...
	node := topology.Node{Name: "ceos", Kind: topology.KindCEOS}
	path := expectedPath(idx, node, sim.RIBEntry{
...
func TestRouteNextHopAddressUsesPeerInterfaceAddress(t *testing.T) {
	idx, err := topology.BuildTopologyIndex(&topology.Topology{
		Nodes: []topology.Node{
			{Name: "local", Interfaces: []topology.Interface{{Name: "eth1", Address: "192.0.2.10/24"}}},
			{Name: "peer", Interfaces: []topology.Interface{{Name: "eth1", Address: "192.0.2.20/24"}}},
		},
		Links: []topology.Link{{
			Name:   "local-peer",
			A:      "local",
...
func TestRouteNextHopAddressUsesRecursiveHopInterfaceAddress(t *testing.T) {
	idx, err := topology.BuildTopologyIndex(&topology.Topology{
		Nodes: []topology.Node{
			{Name: "origin", Interfaces: []topology.Interface{{Name: "eth1", Address: "198.51.100.30/24"}}},
			{Name: "hop", Interfaces: []topology.Interface{{Name: "eth1", Address: "198.51.100.40/24"}, {Name: "eth2", Address: "203.0.113.50/24"}}},
			{Name: "local", Interfaces: []topology.Interface{{Name: "eth1", Address: "203.0.113.60/24"}}},
		},
		Links: []topology.Link{
...
func TestExpectedReflectsRouteMapAttributes(t *testing.T) {
	...
	topo := &topology.Topology{
		Nodes: []topology.Node{
			{
				...
				Kind:     "frr",
				ASN:      65001,
				Prefixes: netaddr.MustPrefixes("10.0.0.0/24"),
				PrefixLists: []topology.PrefixList{{
					Name:  "PL-OUT",
					Rules: []topology.PrefixListRule{{Action: "permit", Prefix: "10.0.0.0/24"}},
				}},
				RoutePolicies: []topology.RoutePolicy{{
					Name:  "SET-MED",
					Rules: []topology.RoutePolicyRule{{Action: "permit", MatchPrefixList: "PL-OUT", SetMED: &med}},
				}},
			...
			},
			{
				...
				RoutePolicies: []topology.RoutePolicy{{
					Name:  "SET-LP",
					Rules: []topology.RoutePolicyRule{{Action: "permit", MatchPrefixList: "PL-IN", SetLocalPref: &lp}},
				}},
				Neighbors: []topology.BGPNeighbor{{
					PeerNode:     "origin",
					RemoteAS:     65001,
		...
		Links: []topology.Link{{Name: "origin-rx", A: "origin", B: "rx", Cost: 1, Subnet: "192.0.2.0/31"}},
	}
	routes := ExpectedForNodes(topo, routing.FromTopology(topo), []topology.Node{{Name: "rx", Kind: "frr"}})
