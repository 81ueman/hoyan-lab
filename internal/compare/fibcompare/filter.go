...
func ComparableRoutes(topo *topology.Topology, routes []NormalizedFIBRoute, opts Options) []NormalizedFIBRoute {
	return AnalyzeComparableRoutes(topo, routes, opts).Routes
}

func AnalyzeComparableRoutes(topo *topology.Topology, routes []NormalizedFIBRoute, opts Options) FilterResult {
...
func normalizeRouteNextHops(idx *topology.TopologyIndex, route NormalizedFIBRoute) []NormalizedFIBNextHop {
	...
	for _, hop := range route.NextHops {
		hop.Interface = topology.ProfileFor(node.Kind).InterfaceProfile().CanonicalInterfaceName(hop.Interface)
...
func comparableNextHops(idx *topology.TopologyIndex, node string, hops []NormalizedFIBNextHop, opts Options) ([]NormalizedFIBNextHop, []UnresolvedNextHop) {
	...
	for _, hop := range hops {
		...
		if opts.AllowUnsupported && !topology.ProfileFor(peerNode.Kind).LiveProfile().SupportsFIBCollection() {
			continue
		}
		out = append(out, hop)
	}
...
func unresolvedNextHop(idx *topology.TopologyIndex, node string, hop NormalizedFIBNextHop) UnresolvedNextHop {
...
func isManagementInterface(idx *topology.TopologyIndex, node, iface string) bool {
	if iface == "" {
		return false
	}
	if idx == nil {
		return topology.ProfileFor(topology.KindFRR).InterfaceProfile().IsManagementInterface(nil, iface)
	}
	n, ok := idx.Node(node)
	if !ok {
		return topology.ProfileFor(topology.KindFRR).InterfaceProfile().IsManagementInterface(nil, iface)
	}
	return topology.ProfileFor(n.Kind).InterfaceProfile().IsManagementInterface(n.Interfaces, iface)
}
...
func peerForNextHopInterface(idx *topology.TopologyIndex, node, iface string) (string, bool) {
	if idx == nil || iface == "" {
		return "", false
	}
	n, ok := idx.Node(node)
	...
	for _, edge := range idx.Adj[topology.NodeID(node)] {
		link := edge.Link
		localIface := link.AIntf
		if link.B == node {
			localIface = link.BIntf
		}
		if topology.ProfileFor(n.Kind).InterfaceProfile().EquivalentInterfaceName(localIface, iface) {
...
func isNodeName(idx *topology.TopologyIndex, name string) bool {
	_, ok := idx.Node(name)
	return ok
}
