...
func TestComparableRoutesCanonicalizesNextHopInterfaceByDeviceProfile(t *testing.T) {
	topo := &topology.Topology{Nodes: []topology.Node{{Name: "srl1", Kind: topology.KindSRLinux}}}
...
func TestComparableRoutesIncludesConnectedClasses(t *testing.T) {
	topo := &topology.Topology{
		Nodes: []topology.Node{
			{Name: "r1", Kind: topology.KindFRR, Interfaces: []topology.Interface{
				{Name: "lo", Address: "10.255.0.1/32"},
				{Name: "eth1", Address: "192.0.2.1/31"},
			}},
			{Name: "r2", Kind: topology.KindFRR, Interfaces: []topology.Interface{{Name: "eth1", Address: "192.0.2.0/31"}}},
		},
		Links: []topology.Link{{Name: "r1-r2", A: "r1", B: "r2", AIntf: "eth1", BIntf: "eth1"}},
	}
	...
	if route := routeByPrefix(filtered, "192.0.2.0/31"); route == nil || route.ConnectedClass != topology.ConnectedRouteClassLink {
		t.Fatalf("link route = %#v", route)
	}
	if route := routeByPrefix(filtered, "10.255.0.1/32"); route == nil || route.ConnectedClass != topology.ConnectedRouteClassLoopback {
...
func TestExpectedForNodesNormalizesModeledFIB(t *testing.T) {
	topo := &topology.Topology{
		Nodes: []topology.Node{
			{Name: "src", Kind: topology.KindFRR, ASN: 65000, Interfaces: []topology.Interface{{Name: "eth1", Address: "192.0.2.1/31"}}, Neighbors: []topology.BGPNeighbor{{
				PeerNode:  "dst",
				RemoteAS:  65001,
				Activated: true,
			}}},
			{Name: "dst", Kind: topology.KindFRR, ASN: 65001, Prefixes: netaddr.MustPrefixes("10.0.0.0/24"), Interfaces: []topology.Interface{{Name: "eth1", Address: "192.0.2.0/31"}}, Neighbors: []topology.BGPNeighbor{{
				PeerNode:  "src",
				RemoteAS:  65000,
				Activated: true,
			}}},
		},
		Links: []topology.Link{{Name: "src-dst", A: "src", B: "dst", AIntf: "eth1", BIntf: "eth1", Cost: 7, Subnet: "192.0.2.0/31"}},
	}
	routes := ExpectedForNodes(topo, routing.FromTopology(topo), []topology.Node{topo.Nodes[0]})
	route := routeByPrefix(routes, "10.0.0.0/24")
	if route == nil {
...
func TestExpectedForNodesResolvesAddressOnlyRecursiveBGPNextHop(t *testing.T) {
	topo := &topology.Topology{
		Nodes: []topology.Node{
			{
				...
				ASN:        65004,
				Interfaces: []topology.Interface{{Name: "eth2", Address: "198.18.2.6/31"}},
				Routes: []topology.ConfiguredRoute{{
					Prefix:  netaddr.MustPrefix("10.119.2.2/32"),
					NextHop: "198.18.2.7",
			...
			},
			{
				...
				Kind:       topology.KindFRR,
				ASN:        65004,
				Prefixes:   netaddr.MustPrefixes("10.119.0.0/24"),
				Interfaces: []topology.Interface{{Name: "eth1", Address: "198.18.2.7/31"}},
				Neighbors: []topology.BGPNeighbor{{
					PeerNode:     "hz-edge1",
					RemoteAS:     65004,
					Activated:    true,
		...
		Links: []topology.Link{{Name: "hz1-hz2", A: "hz-edge1", B: "hz-edge2", AIntf: "eth2", BIntf: "eth1", Subnet: "198.18.2.6/31"}},
	}
	routes := ExpectedForNodes(topo, routing.FromTopology(topo), []topology.Node{topo.Nodes[0]})
...
func TestExpectedForNodesSuppressesSRLinuxLoopbackConnectedFIB(t *testing.T) {
	...
	routes := ExpectedForNodes(topo, routing.FromTopology(topo), topo.Nodes)
...
func TestExpectedForNodesKeepsLocalBlackholeAndSuppressesSamePrefixBGPFIB(t *testing.T) {
	...
	topo := &topology.Topology{Nodes: []topology.Node{{
		...
		Kind:     topology.KindFRR,
		ASN:      65000,
		Prefixes: []netaddr.Prefix{prefix},
		Routes:   []topology.ConfiguredRoute{{Prefix: prefix, Kind: topology.RouteSourceBlackhole, Interface: "Null0"}},
	}}}
	routes := ExpectedForNodes(topo, routing.FromTopology(topo), topo.Nodes)
...
func TestCompareReportsDuplicateRouteConflictForConnectedClass(t *testing.T) {
	result := Compare([]NormalizedFIBRoute{
		{Node: "r1", VRF: "default", AFI: "ipv4", Prefix: "192.0.2.0/31", Protocol: "connected", ConnectedClass: topology.ConnectedRouteClassLink, Installed: true},
		{Node: "r1", VRF: "default", AFI: "ipv4", Prefix: "192.0.2.0/31", Protocol: "connected", ConnectedClass: topology.ConnectedRouteClassLoopback, Installed: true},
	}, nil)
...
func TestCollectRejectsUnsupportedNodes(t *testing.T) {
	_, err := Collect(context.Background(), fakeRunner{}, []topology.Node{{Name: "unknown1", Kind: topology.DeviceKind("unknown")}}, Options{})
...
func TestCollectFRRKernelRoutes(t *testing.T) {
	...
	routes, err := Collect(context.Background(), runner, []topology.Node{{Name: "r1", Kind: topology.KindFRR, ContainerName: "clab-test-r1"}}, Options{})
	if err != nil {
		t.Fatalf("Collect() error = %v", err)
	}
...
func TestCollectAllSupportedKinds(t *testing.T) {
	...
	routes, err := Collect(context.Background(), runner, []topology.Node{
		{Name: "frr", Kind: topology.KindFRR, ContainerName: "frr1"},
		{Name: "ceos", Kind: topology.KindCEOS, ContainerName: "ceos1"},
		{Name: "srl", Kind: topology.KindSRLinux, ContainerName: "srl1"},
	}, Options{})
	if err != nil {
		t.Fatalf("Collect() error = %v", err)
	}
...
func TestCollectSRLinuxUsesRouteDetailPeerGateway(t *testing.T) {
	...
	routes, err := Collect(context.Background(), runner, []topology.Node{{Name: "core-gz", Kind: topology.KindSRLinux, ContainerName: "srl1"}}, Options{})
	if err != nil {
		t.Fatalf("Collect() error = %v", err)
	}
	route := routeByPrefix(routes, "10.4.0.0/16")
...
func TestComparableRoutesFiltersNonBGPAndUnsupportedNextHops(t *testing.T) {
	topo := &topology.Topology{
		Nodes: []topology.Node{
			{Name: "r1", Kind: topology.KindFRR, Interfaces: []topology.Interface{{Name: "eth1", Address: "192.0.2.1/31"}, {Name: "eth2", Address: "198.51.100.1/31"}}},
			{Name: "r2", Kind: topology.KindFRR, Interfaces: []topology.Interface{{Name: "eth1", Address: "192.0.2.0/31"}}},
			{Name: "nos1", Kind: topology.DeviceKind("unknown"), Interfaces: []topology.Interface{{Name: "eth1", Address: "198.51.100.0/31"}}},
		},
		Links: []topology.Link{
...
func TestAnalyzeComparableRoutesReportsManagementFallback(t *testing.T) {
	topo := &topology.Topology{
		Nodes: []topology.Node{
			{Name: "r1", Kind: topology.KindFRR, Interfaces: []topology.Interface{{Name: "eth1", Address: "192.0.2.1/31"}}},
			{Name: "r2", Kind: topology.KindFRR, Interfaces: []topology.Interface{{Name: "eth1", Address: "192.0.2.0/31"}}},
		},
		Links: []topology.Link{{Name: "r1-r2", A: "r1", B: "r2", AIntf: "eth1", BIntf: "eth1"}},
	}
...
func TestComparableRoutesKeepsSRLinuxDetailNextHopAddress(t *testing.T) {
	topo := &topology.Topology{
		Nodes: []topology.Node{
			{Name: "core-gz", Kind: topology.KindSRLinux, Interfaces: []topology.Interface{{Name: "ethernet-1/4.0", Address: "198.18.20.4/31"}}},
			{Name: "core-hz", Kind: topology.KindFRR, Interfaces: []topology.Interface{{Name: "eth3", Address: "198.18.20.5/31"}}},
		},
		Links: []topology.Link{{Name: "gz-hz", A: "core-gz", B: "core-hz", AIntf: "e1-4", BIntf: "eth3"}},
	}
	routes := []NormalizedFIBRoute{{
		Node:     "core-gz",
		VRF:      "default",
