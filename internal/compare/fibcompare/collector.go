...
func Collect(ctx context.Context, runner Runner, nodes []topology.Node, opts Options) ([]NormalizedFIBRoute, error) {
	...
	for _, kind := range topology.RegisteredDeviceKinds() {
		collectorID, ok := topology.ProfileFor(kind).LiveProfile().FIBCollector()
		if !ok {
			continue
		}
...
func SupportedNodes(nodes []topology.Node) []topology.Node {
	...
	collectors := fibCollectorsByID()
	for _, n := range nodes {
		collectorID, ok := topology.ProfileFor(n.Kind).LiveProfile().FIBCollector()
...
func unsupportedNodes(nodes []topology.Node) []string {
	var out []string
	collectors := fibCollectorsByID()
	for _, n := range nodes {
		collectorID, ok := topology.ProfileFor(n.Kind).LiveProfile().FIBCollector()
...
func fibCollectorsByID() map[topology.LiveCollectorID]Collector {
	return map[topology.LiveCollectorID]Collector{
...
func (frrCollector) Collect(ctx context.Context, runner Runner, nodes []topology.Node) ([]NormalizedFIBRoute, error) {
	var out []NormalizedFIBRoute
	for _, n := range nodes {
...
func (ceosCollector) Collect(ctx context.Context, runner Runner, nodes []topology.Node) ([]NormalizedFIBRoute, error) {
	var out []NormalizedFIBRoute
	for _, n := range nodes {
		containerName := n.RuntimeName()
...
func (srlinuxCollector) Collect(ctx context.Context, runner Runner, nodes []topology.Node) ([]NormalizedFIBRoute, error) {
	var out []NormalizedFIBRoute
	for _, n := range nodes {
		containerName := n.RuntimeName()
